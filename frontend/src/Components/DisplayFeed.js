...
import '../CSS for Components/DisplayFeed.css';
...
function DisplayFeed() {
    ...
    async function get_profile_photo() {
        let dataa = {
            user_name: user_login_info_from_cache.user_name,
            profile_photo: "NULL"
        };
        try {
    ...
    async function get_all_posts(loader_text) {
        try {
            ...
            const response = await fetch("http://localhost:4000/api/get_feed/" + user_login_info_from_cache.user_name + "/" + offset, { method: 'GET' });
